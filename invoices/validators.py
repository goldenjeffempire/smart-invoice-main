...
def validate_tax_rate(value):
    ...
    try:
        ...
        if decimal_value < 0 or decimal_value > 100:
            raise ValidationError(
                _('Tax rate must be between 0 and 100 percent.'),
                code='invalid_tax_rate'
            )
    except (ValueError, InvalidOperation):
        raise ValidationError(
            _('Tax rate must be a valid number.'),
            code='invalid_decimal'
        )
...
def validate_invoice_date(value):
    ...
    if value > date.today() + timedelta(days=365):
        raise ValidationError(
            _('Invoice date cannot be more than 1 year in the future.'),
...
def validate_hex_color(value):
    ...
    if not re.match(pattern, value):
        raise ValidationError(
            _('Enter a valid hex color code (e.g., #6366f1 or #fff).'),
            code='invalid_color'
        )
...
def validate_email_domain(value):
    ...
    if '.' not in domain:
        raise ValidationError(
            _('Email address must have a valid domain (e.g., @example.com).'),
            code='missing_tld'
        )
...
class InvoiceBusinessRules:
    ...
    def validate_due_date(invoice_date, due_date):
        ...
        if due_date and invoice_date and due_date < invoice_date:
            raise ValidationError(
                _('Due date must be on or after the invoice date.'),
    ...
    def validate_line_items(line_items_data):
        ...
        if not line_items_data:
            raise ValidationError(
                _('At least one line item is required.'),
                code='no_line_items'
            )
